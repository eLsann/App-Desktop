...
class ApiClient:
    ...
    # ---------------- Device ----------------
    def recognize(self, jpg_bytes: bytes) -> dict:
        """Send face recognition request to API with improved error handling"""
        if not jpg_bytes:
        ...
        files = {"file": ("face.jpg", jpg_bytes, "image/jpeg")}

        try:
            logger.debug(f"Sending recognition request: {len(jpg_bytes)} bytes")
            r = self.session.post(url, headers=headers, files=files, timeout=self.timeout)
            
            if r.status_code == 401:
                raise RuntimeError("Authentication failed - invalid device token")
            ...
            result = r.json()
            logger.debug(f"Recognition response: {result.get('status', 'unknown')}")
            return result
            ...
            logger.error(f"Unexpected error in recognize: {str(e)}")
            raise
